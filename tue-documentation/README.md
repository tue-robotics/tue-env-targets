...
## Generation
Make sure all documentation dependencies are installed by running `irohms-get install/update` with the `--doc-depend` option. To generate the documetation run `irohms-make-documentation` it wraps `catkin` and therefore accepts all `catkin` arguments. After generating the documentation, you can open the documentation in your browser:
```bash
...
## Python
Python documentation is generated by [Sphinx](http://sphinx-doc.org/). A fully dynamic Sphinx configuration is available: [config](https://github.com/irohms-robotics/irohms_documentation_python.git). Add this to your package as a submodule,
```bash
git submodule add https://github.com/irohms-robotics/irohms_documentation_python.git FOLDER -b master
```

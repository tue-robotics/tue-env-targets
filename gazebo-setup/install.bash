...
then
    ...
    then
        ...
        tue-install-debug "Downloading gpg key of Gazebo repo with fingerprint '${key_fingerprint}'."
        curl -fsSL ${key_url} | sudo gpg --dearmor --yes -o "${key_file}"
    else
        tue-install-debug "GPG key of Gazebo repo with fingerprint '${key_fingerprint}' already exists, so not installing it."
    fi
    ...
    then
        tue-install-debug "Updating Gazebo sources to apt-get"
        source_needs_to_be_added=true
    else

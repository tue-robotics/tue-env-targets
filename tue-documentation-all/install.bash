...
local CUCR_INSTALL_SKIP_ROS_DEPS=normal
local targets
...
do
    ...
    target_yaml_file="$CUCR_ENV_TARGETS_DIR"/"$target"/install.yaml
    ...
    local cmds
    if cmds=$("$CUCR_INSTALL_SCRIPTS_DIR"/parse-install-yaml.py "$target_yaml_file")
    then
        for cmd in $cmds
        do
